...
#include "kudu/util/jsonbuilder.h"
...
TEST_F(JsonBuilderTest, TestBasic) {
  ...
  ASSERT_EQ(ej.Get("1").GetBool(), true);
  ASSERT_EQ(ej.Get("2").GetInt(), kint32min);
  ASSERT_EQ(ej.Get("3").GetUint(), kuint32max);
  ASSERT_EQ(ej.Get("4").GetInt64(), kint64min);
  ASSERT_EQ(ej.Get("5").GetUint64(), kuint64max);
  ASSERT_EQ(ej.Get("6").GetDouble(), 1.0);
  ASSERT_EQ(string(ej.Get("7").GetString()), "string");

  Value& v = ej.value();

  ASSERT_EQ(v["1"].GetBool(), true);
  ASSERT_EQ(v["2"].GetInt(), kint32min);
  ASSERT_EQ(v["3"].GetUint(), kuint32max);
  ASSERT_EQ(v["4"].GetInt64(), kint64min);
  ASSERT_EQ(v["5"].GetUint64(), kuint64max);
...
TEST_F(JsonBuilderTest, TestCompactSyntax) {
  ...
  ASSERT_EQ(ej["nested_array"][0].GetInt(), 1);
  ASSERT_EQ(ej["nested_array"][1].GetInt(), 2);
}
