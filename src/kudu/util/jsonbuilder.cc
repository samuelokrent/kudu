...
uint32_t EasyJson::GetUint() const {
  if (value_.IsUint())
...
int64_t EasyJson::GetInt64() const {
  if (value_.IsInt64())
...
uint64_t EasyJson::GetUint64() const {
  if (value_.IsUint64())
...
double EasyJson::GetDouble() const {
  if (value_.IsDouble())
...
const char* EasyJson::GetString() const {
  if (value_.IsString())
